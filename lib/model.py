    ...
    def test1(self):
        ...
        with torch.no_grad():
            ...
            if self.opt.load_weights:
                netg_path = "./output/{}/{}/train/weights/netG.pth".format(self.name().lower(), self.opt.dataset)
                netg_pretrained_dict = torch.load(netg_path)['state_dict']

                try:
                    self.netg.load_state_dict(netg_pretrained_dict)
                except IOError:
                    raise IOError("netG weights not found")
                netd_path = "./output/{}/{}/train/weights/netD.pth".format(self.name().lower(), self.opt.dataset)
                netd_pretrained_dict = torch.load(netd_path)['state_dict']

                try:
                    self.netd.load_state_dict(netd_pretrained_dict)
                except IOError:
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                out,_ = self.netd(self.fake)
                print(out.size())
                # error = torch.mean(torch.pow((latent_i-latent_o), 2), dim=1)
                time_o = time.time()

                self.an_scores[i * self.opt.batchsize: i * self.opt.batchsize + out.size(0)] = out.reshape(out.size(0))
                self.gt_labels[i*self.opt.batchsize : i*self.opt.batchsize+out.size(0)] = self.gt.reshape(out.size(0))
                print(out.reshape(out.size(0)))
                print(self.gt.reshape(out.size(0)))
                ...
                if self.opt.save_test_images:
                    ...
                    vutils.save_image(real, '%s/real_%03d.png' % (dst, i+1), normalize=True,nrow=4,padding=0)
                    vutils.save_image(fake, '%s/fake_%03d.png' % (dst, i+1), normalize=True,nrow=4,padding=0)
